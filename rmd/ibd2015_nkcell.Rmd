...
The actual running time per task ranges from 5 minutes to 8.9 hours, with median being 2.1 hours.

Each task of the job array outputs results in a
...
stores RSS-NET results based on the 66-th row of hyper-parameter data frame.
...
variational lower bound from my previous run with that of the current run.
...
Let's perform a similar reproducibility check for gene-level results.
...
reported in [Zhu et al (2020)][] are essentially 684 modified rerun of the example above
(with different input GWAS and/or network data, and different hyper-parameter grids).
