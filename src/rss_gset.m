function [logw,alpha,mu,s,param] = rss_gset(data,hyper,options)
...
%	param: grid of hyper-parameters [theta0 theta sigma0 sigma], num_hyper by 4 
  ...
  % Scenario 2: RSS-NET is applied to a subset of SNPs. 
  subset_genome = ismember('snps',sumstats_info);
  subset_genome = subset_genome && (data.num_snp_fitted ~= data.num_snp);

  if subset_genome
    snps = sumstats_data.snps; %#ok<NASGU>

    if (length(sumstats_data.snps) ~= data.num_snp_fitted)
      error('Inconsistent number of SNPs in the subset ...\n');
    end

    fprintf('This RSS-NET analysis uses %d SNPs in the subset ...\n',data.num_snp_fitted);
  end
  ...
  % Load SNP-level annotations based on the given gene set.
  % 1: inside the gene set; 0: outside the gene set.
  snp2gset = matfile(data.snp2gset_file);
  gsetflag = double(snp2gset.val);
  gsetflag = gsetflag(:);
  clear snp2gset;
  ...
  for k=1:num_hyper
    ...
    theta0  = param(k,1);
    theta   = param(k,2);
    logodds = make_logodds(data.snp2gset_file,theta0,theta,options);
    ...
    if induced_flag
      ...
      [sigma0,sigma] = make_sigma(data,gsetflag,logodds,eta,rho);
    ...
    sigbeta = sqrt(sigma0^2 + sigma^2*gsetflag);  
